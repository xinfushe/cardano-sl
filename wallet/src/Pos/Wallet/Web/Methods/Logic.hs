...
getAccount nm accId = do
    ...
    accMod <- txMempoolToModifier ws mps . (eskToWalletDecrCredentials nm) =<< findKey accId
...
getAccountsIncludeUnready nm ws mps includeUnready mCAddr = do
    ...
    concatForM (HM.toList groupedAccIds) $ \(wid, walAccIds) -> do
      accMod <- txMempoolToModifier ws mps . (eskToWalletDecrCredentials nm) =<< findKey wid
...
getAccounts
    :: MonadWalletLogicRead ctx m
    => Maybe Int32 -> Maybe (CId Wal) -> m [CAccount]
...
getWalletIncludeUnready nm ws mps includeUnready cAddr = do
    ...
    accMod     <- txMempoolToModifier ws mps . (eskToWalletDecrCredentials nm) =<< findKey cAddr
...
getWallet :: MonadWalletLogicRead ctx m => Maybe Int32 -> CId Wal -> m CWallet
getWallet nm wid = do
    ws <- askWalletSnapshot
    mps <- withTxpLocalData getMempoolSnapshot
    getWalletIncludeUnready nm ws mps False wid

getWallets ::  MonadWalletLogicRead ctx m => Maybe Int32 -> m [CWallet]
getWallets nm = do
    ws <- askWalletSnapshot
    mps <- withTxpLocalData getMempoolSnapshot
...
newAccountIncludeUnready
    :: MonadWalletLogic ctx m
    => Maybe Int32 -> Bool -> AddrGenSeed -> PassPhrase -> CAccountInit -> m CAccount
newAccountIncludeUnready nm includeUnready addGenSeed passphrase CAccountInit {..} = do
    ...
    accMod <- txMempoolToModifier ws mps . (eskToWalletDecrCredentials nm) =<< findKey caInitWId
    -- check wallet exists
...
newAccount
    :: MonadWalletLogic ctx m
    => Maybe Int32 -> AddrGenSeed -> PassPhrase -> CAccountInit -> m CAccount
newAccount nm = newAccountIncludeUnready nm False

createWalletSafe
    :: MonadWalletLogic ctx m
    => Maybe Int32 -> CId Wal -> CWalletMeta -> Bool -> m CWallet
...
deleteWallet :: MonadWalletLogic ctx m => Maybe Int32 -> CId Wal -> m NoContent
...
updateWallet :: MonadWalletLogic ctx m => Maybe Int32 -> CId Wal -> CWalletMeta -> m CWallet
updateWallet nm wId wMeta = do
    db <- askWalletDB
    setWalletMeta db wId wMeta
    getWallet nm wId

updateAccount :: MonadWalletLogic ctx m => Maybe Int32 -> AccountId -> CAccountMeta -> m CAccount
...
changeWalletPassphrase
    :: MonadWalletLogic ctx m
    => Maybe Int32 -> CId Wal -> PassPhrase -> PassPhrase -> m NoContent
