...
mkWallet
    ...
    => Maybe Int32 -> PassPhrase -> CWalletInit -> Bool -> m (EncryptedSecretKey, CId Wal)
...
newWallet :: L.MonadWalletLogic ctx m => Maybe Int32 -> PassPhrase -> CWalletInit -> m CWallet
...
restoreWalletFromSeed :: ( L.MonadWalletLogic ctx m
                         ...
                         ) => Maybe Int32 -> PassPhrase -> CWalletInit -> m CWallet
...
restoreWallet :: ( L.MonadWalletLogic ctx m
                 , MonadUnliftIO m
                 , HasLens SyncQueue ctx SyncQueue
                 ) => Maybe Int32 -> EncryptedSecretKey -> m CWallet
...
restoreWalletFromBackup :: ( L.MonadWalletLogic ctx m
                           , MonadUnliftIO m
                           , HasLens SyncQueue ctx SyncQueue
                           ) => Maybe Int32 -> WalletBackup -> m CWallet
