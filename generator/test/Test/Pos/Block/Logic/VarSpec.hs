  ...
  where
    go rnm = withStaticConfigurations $ \txpConfig _ -> do
        describe "Block.Logic.VAR" $ modifyMaxSuccess (min 4) $ do
            describe "verifyBlocksPrefix" $ verifyBlocksPrefixSpec rnm txpConfig
            describe "verifyAndApplyBlocks" $ verifyAndApplyBlocksSpec rnm txpConfig
            describe "applyBlocks" applyBlocksSpec
            describe "Block.Event" $ do
                describe "Successful sequence" $ blockEventSuccessSpec rnm txpConfig
                describe "Apply through epoch" $ applyThroughEpochSpec rnm txpConfig 0
                describe "Apply through epoch" $ applyThroughEpochSpec rnm txpConfig 4
                describe "Fork - short" $ singleForkSpec rnm txpConfig ForkShort
                describe "Fork - medium" $ singleForkSpec rnm txpConfig ForkMedium
                describe "Fork - deep" $ singleForkSpec rnm txpConfig ForkDeep
...
verifyBlocksPrefixSpec
    ...
    -> TxpConfiguration
    -> Spec
verifyBlocksPrefixSpec rnm txpConfig = do
    blockPropertySpec verifyEmptyMainBlockDesc (verifyEmptyMainBlock rnm txpConfig)
    blockPropertySpec verifyValidBlocksDesc (verifyValidBlocks rnm txpConfig)
  where
...
verifyEmptyMainBlock :: HasConfigurations
                     ...
                     -> TxpConfiguration
                     -> BlockProperty ()
verifyEmptyMainBlock rnm txpConfig = do
...
verifyValidBlocks
    ...
    -> TxpConfiguration
    -> BlockProperty ()
verifyValidBlocks rnm txpConfig = do
...
verifyAndApplyBlocksSpec :: HasStaticConfigurations
                         ...
                         -> TxpConfiguration
                         -> Spec
verifyAndApplyBlocksSpec rnm txpConfig =
    blockPropertySpec applyByOneOrAllAtOnceDesc (applyByOneOrAllAtOnce rnm txpConfig applier)
  where
...
applyByOneOrAllAtOnce
    ...
    -> BlockProperty ()
applyByOneOrAllAtOnce rnm txpConfig applier = do
    bpGoToArbitraryState
    let nm = makeNetworkMagic rnm dummyProtocolMagic
...
blockEventSuccessSpec :: HasStaticConfigurations
                      ...
                      -> TxpConfiguration
                      -> Spec
blockEventSuccessSpec rnm txpConfig =
    blockPropertySpec blockEventSuccessDesc (blockEventSuccessProp rnm txpConfig)
  where
...
blockPropertyScenarioGen rnm txpConfig m = do
...
blockEventSuccessProp :: HasConfigurations => RequiresNetworkMagic -> TxpConfiguration
                      -> BlockProperty ()
blockEventSuccessProp rnm txpConfig = do
    scenario <- blockPropertyScenarioGen rnm txpConfig genSuccessWithForks
    ...
    runBlockScenarioAndVerify rnm txpConfig scenario'
...
runBlockScenarioAndVerify
    ...
    -> BlockScenario
    -> BlockProperty ()
runBlockScenarioAndVerify rnm txpConfig bs =
    verifyBlockScenarioResult =<< lift (runBlockScenario rnm txpConfig bs)
...
applyThroughEpochSpec
    ...
    -> Int
    -> Spec
applyThroughEpochSpec rnm txpConfig afterCross = do
    blockPropertySpec applyThroughEpochDesc (applyThroughEpochProp rnm txpConfig afterCross)
  where
...
applyThroughEpochProp :: HasConfigurations
                      ...
                      -> Int
                      -> BlockProperty ()
applyThroughEpochProp rnm txpConfig afterCross = do
    scenario <- blockPropertyScenarioGen rnm txpConfig $ do
        let
            approachEpochEdge =
...
singleForkSpec :: HasStaticConfigurations
               ...
               -> ForkDepth
               -> Spec
singleForkSpec rnm txpConfig fd = do
    blockPropertySpec singleForkDesc (singleForkProp rnm txpConfig fd)
  where
    singleForkDesc =
...
singleForkProp :: HasConfigurations
               ...
               -> ForkDepth
               -> BlockProperty ()
singleForkProp rnm txpConfig fd = do
    scenario <- blockPropertyScenarioGen rnm txpConfig $ genSingleFork fd
    runBlockScenarioAndVerify rnm txpConfig scenario
