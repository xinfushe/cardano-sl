...
genBlocksInForest pm nm txpConfig secrets bootStakeholders =
    traverse $ mapRandT withClonedGState .
    genBlocksInTree pm nm txpConfig secrets bootStakeholders
...
genBlocksInTree pm nm txpConfig secrets bootStakeholders blockchainTree = do
    ...
    blocks <- genBlocks pm nm txpConfig blockGenParams maybeToList
    ...
    forestBlocks <- genBlocksInForest pm nm txpConfig secrets bootStakeholders blockchainForest
...
genBlocksInStructure ::
    ...
    -> t Path
    -> RandT g m (t Blund)
genBlocksInStructure pm nm txpConfig secrets bootStakeholders annotations s = do
    let
    ...
    blockForest :: BlockchainForest Blund <-
        genBlocksInForest pm nm txpConfig secrets bootStakeholders descForest
    let
        getBlock :: Path -> Blund
