...
withNewWalletFixture :: NetworkMagic
                     -> (  Keystore.Keystore
                     ...
                     -> PropertyM IO a
withNewWalletFixture nm cc = withPassiveWalletFixture nm (prepareFixtures nm) cc
    ...
    go rnm = describe "Wallets" $ do
        ...
        describe "DeleteWallet" $ do
            describe "Wallet deletion (wallet layer)" $ do
                ...
                prop "works as expected in the happy path scenario" $ withMaxSuccess 50 $ do
                    monadicIO $ do
                        withNewWalletFixture nm $ \_ layer _ Fixture{..} -> do
            ...
            describe "Wallet deletion (kernel)" $ do
                prop "correctly deletes the ESK in the keystore" $ withMaxSuccess 50 $
                    monadicIO $ do
                        withNewWalletFixture nm $ \ks _ wallet Fixture{..} -> do
                            liftIO $ do
        ...
        describe "UpdateWalletPassword" $ do
            ...
            describe "Wallet update password (kernel)" $ do
                prop "correctly replaces the ESK in the keystore" $ withMaxSuccess 50 $
                    monadicIO $ do
                        newPwd <- pick arbitrary
                        withNewWalletFixture nm $ \ keystore _ wallet Fixture{..} -> do
                            let wid = WalletIdHdRnd fixtureHdRootId
                            oldKey <- Keystore.lookup nm wid keystore
                            res <- Kernel.updatePassword nm
                                                        wallet
                                                        fixtureHdRootId
