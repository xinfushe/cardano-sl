...
walletServer :: (HasConfigurations, HasCompileInfo)
             ...
             -> RunMode
             -> Server WalletAPI
walletServer natV0 pm nm txpConfig diffusion ntpStatus runMode =
         v0Handler
    :<|> v0Handler
    :<|> v1Handler
    :<|> internalHandler
  where
    v0Handler       = V0.handlers natV0 pm nm txpConfig diffusion ntpStatus
    v1Handler       = V1.handlers natV0 pm nm txpConfig diffusion ntpStatus
